...
-- 체크리스트 개별 항목 수집
CREATE TABLE item_collect_stat (
                                   stat_id BIGINT PRIMARY KEY AUTO_INCREMENT,
                                   source_checklist_id BIGINT NOT NULL,
                                   template_item_id BIGINT NULL,
                                   item_title VARCHAR(150) NOT NULL,
                                   ...
                                   FOREIGN KEY (source_checklist_id) REFERENCES user_checklist(user_checklist_id) ON DELETE CASCADE,
                                   FOREIGN KEY (template_item_id) REFERENCES checklist_template_item(template_item_id) ON DELETE SET NULL,

                                   INDEX idx_item_collect_count (template_item_id, collect_count DESC),
                                   INDEX idx_checklist_popularity (source_checklist_id, collect_count DESC)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
